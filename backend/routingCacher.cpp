    ...
    std::string toJson(std::unordered_map<StopId, StopState> map) {
        ...
        std::for_each(map.begin(), map.end(), [&finalObj](const std::pair<StopId, StopState> &entry) {
            ...
            std::transform(entry.second.incoming.begin(), entry.second.incoming.end(),
                           std::back_inserter(incomingTrips),
                           [](IncomingTrip trip) {
            ...
            boost::json::value val = {{"time",     entry.second.travelTime},
 ...
 */
    ...
    std::unordered_map<StopId, ParsedStopState> fromJson(const std::string &json) {
        ...
        std::transform(rootObj.begin(), rootObj.end(), std::inserter(res, res.end()),
                       [](boost::json::key_value_pair pair) {
                           ...
                           std::transform(trips.begin(), trips.end(), std::back_inserter(incomings),
                                          [](boost::json::value val) {
                                              return ParsedIncomingTrip{val.at("from").to_number<uint64_t>(),
                                                                        val.at("trip").to_number<uint64_t>()};
                                          });
    ...
    void test() {
        ...
        std::cout << std::endl << "[TEST] Testing with Timetable and routing algorithm... loading timetable"
                  << std::endl;
